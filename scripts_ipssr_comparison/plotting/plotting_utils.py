...
from sklearn.metrics import PrecisionRecallDisplay, RocCurveDisplay, log_loss
...
def precision_recall_plot(y_test, y_prob, file_name, plot_dir, plot=False):
    ...
    if plot: plt.savefig(plot_dir + "/" + file_name  + ".pdf")
    plt.clf()
...
def classification_plot(test_data_df, y_test, output_file, plot_dir, max_quarters):
    ...
    # Show the plot
    plt.tight_layout()
    plt.savefig(plot_dir + "/classification_plot.pdf")
    plt.clf()
...
def plot_kaplan_meier(km, results, plot_dir):
    ...
    fig.savefig(plot_dir + "/kaplan_meier_train.pdf")
    plt.clf()


def plot_tte(tte_gbm, tte_ipssr, meanlineprops, medianprops, colors, plot_dir):
    print("PLOTTING BRIER BY TTE")
    plt.clf()
    plt.title("Brier Score for positive class depending on time-to-event", fontsize=14)
    for i in range(4):
        bp1 = plt.boxplot([tte_gbm[:, i], tte_ipssr[:, i]], positions=[i * 4, (i * 4) + 2], patch_artist=True,
    ...
    hR, = plt.plot([1, 1], [0.2, 0.2], '-', color='tomato')
    plt.legend((hB, hR), ('GBM', 'IPSS-R'), loc="upper left")
    hB.set_visible(False)
    hR.set_visible(False)
    plt.ylabel("Brier Score", fontsize=12)
    plt.xlabel("days to event", fontsize=12)
    plt.tight_layout()
    plt.savefig(plot_dir + "/tte_analysis.pdf")
...
def plot_ratio_over_time(df, lengths, meanlineprops, medianprops, colors, plot_dir):
    ...
    plt.tight_layout()
    plt.savefig(plot_dir + "/stability_over_time_ratio.pdf")


def plot_brier(results_gbm, results_km, meanlineprops, medianprops, plot_dir):
    ...
    for i in range(3):
        ax = axs[i]
        bp1 = ax.boxplot([results_gbm[:, i], results_km[:, i]], labels=['GBM', 'IPSS-R'], notch=False, showmeans=True,
    ...
    plt.ylabel("Brier Score", fontsize=12)
    plt.tight_layout()
    fig.savefig(plot_dir + "/brier_score.pdf")


def plot_single_index_result_metric(results_gbm, results_km, metric_index, metric_name, meanlineprops, medianprops, plot_dir, random_baseline = 0.5):
    plt.clf()
    print(f"PLOTTING {metric_name}")
    # plt.title("AUROC for IPSS-R and GBM model", fontsize=14)
    bp1 = plt.boxplot([results_gbm[:, metric_index], results_km[:, metric_index]], labels=["GBM", "IPSS-R"], notch=False, showmeans=True,
                      meanprops=meanlineprops, medianprops=medianprops)
    if metric_name == "AUROC": plt.axhline(0.5, linestyle="--")
    if metric_name == "AUPRC": plt.axhline(random_baseline, linestyle="--")
    print("Median:", [item.get_ydata() for item in bp1['medians']])
    print("Means:", [item.get_ydata() for item in bp1['means']])
    print("Whiskers:", [item.get_ydata() for item in bp1['whiskers']])
    plt.ylim(0, 1)
    ...
    plt.tight_layout()
    plt.savefig(plot_dir + f"/{metric_name}.pdf")
...
def plot_metric_by_length(df, metric, metric_title, lengths, meanlineprops, medianprops, colors, plot_dir):
    ...
    f.set_figwidth(12)
    for i, l in enumerate(lengths):
        bp1 = plt.boxplot([get_length_metric(df, f"{metric}_gbm", l),
                           get_length_metric(df, f"{metric}_ipssr", l)], positions=[i * 8, (i * 8) + 2],
    ...
    hR, = plt.plot([1, 1], '-', color='tomato')
    plt.legend((hB, hR), ('GBM', 'IPSS-R'), loc="lower left")
    hB.set_visible(False)
    hR.set_visible(False)
    plt.xlim(left=-5)
    plt.ylim(0, 1)
    ...
    plt.tight_layout()
    plt.savefig(plot_dir + f"/stability_over_time_{metric}.pdf")


def plot_average_metric_by_length(df, metric, metric_title, lengths, colors, plot_dir):
    aurocs_gbm_mean, aurocs_ipssr_mean = [], []
    for l in lengths:
        aurocs_gbm_mean.append(np.mean(get_length_metric(df, f"{metric}_gbm", l)))
        aurocs_ipssr_mean.append(np.mean(get_length_metric(df, f"{metric}_ipssr", l)))
    ...
    plt.plot(lengths, aurocs_ipssr_mean, linestyle="-", marker="o", color=colors[1], label="IPSS-R")
    plt.legend(loc="lower left")
    plt.ylim(0, 1)
    plt.tight_layout()
    plt.savefig(plot_dir + f"/stability_over_time_mean_{metric}.pdf")

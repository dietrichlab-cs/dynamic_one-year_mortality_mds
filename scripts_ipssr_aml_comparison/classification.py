...
def classify(X_train, y_train, X_test, y_test, feature_matrix, model_params = MODEL_PARAMS, verbose=False, output_file=None):
    ...
    X_train = X_train.drop(columns=['ipssr_age'], errors="ignore")
...
def cross_validation(feature_matrix, feature_matrix_constants, kaplan_meier_input, feature_importance_file):
    ...
    for res in results:
        ...
        results_ipssr = np.vstack((results_ipssr, res['metrics_ipssr']))
    ...
    colors = ['cornflowerblue', 'tomato', 'lightgreen']
    ...
    plot_brier(results_ipssr, meanlineprops, medianprops, PLOT_DIR, "ipssr")
    ...
    plot_single_index_result_metric(results_ipssr, 3, "AUROC", meanlineprops, medianprops, PLOT_DIR, "ipssr")
    ...
    plot_metric_by_length(length_metrics_full_df, "auroc", "AUROC score", ["gbm", "baseline", "ipssr"], lengths, meanlineprops, medianprops, colors, PLOT_DIR)
    plot_metric_by_length(length_metrics_full_df, "auprc", "AUPRC score", ["gbm", "baseline", "ipssr"], lengths, meanlineprops, medianprops, colors, PLOT_DIR)
    plot_metric_by_length(length_metrics_full_df, "mse", "Brier score", ["gbm", "baseline", "ipssr"], lengths, meanlineprops, medianprops, colors, PLOT_DIR)
    plot_average_metric_by_length(length_metrics_full_df, "auroc", "AUROC score", ["gbm", "baseline", "ipssr"], lengths, colors, PLOT_DIR)
    plot_average_metric_by_length(length_metrics_full_df, "auprc", "AUPRC score", ["gbm", "baseline", "ipssr"], lengths, colors, PLOT_DIR)
    plot_average_metric_by_length(length_metrics_full_df, "mse", "Brier score", ["gbm", "baseline", "ipssr"], lengths, colors, PLOT_DIR)
    plot_average_metrics_by_length(length_metrics_full_df, ["gbm", "baseline", "ipssr"], lengths, colors, PLOT_DIR)
    plot_metrics_by_length(length_metrics_full_df, ["gbm", "baseline", "ipssr"], lengths, colors, PLOT_DIR)
...
def train_and_classify(feature_matrix_csv, feature_matrix_constants_csv, kaplan_meier_input_csv, km_easix, classification_output, feature_output):
...
train_and_classify(DATASET_DIR + "/survival_feature_matrix.csv",
        ...
        DATASET_DIR + "/kaplan_meier_input.csv", DATASET_DIR + "/kaplan_meier_input_easix.csv",

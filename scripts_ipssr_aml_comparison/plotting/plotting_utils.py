...
def plot_single_index_result_metric(results_gbm, metric_index, metric_name, meanlineprops, medianprops, plot_dir,filename, random_baseline = 0.5):
    ...
    plt.tight_layout()
    plt.savefig(plot_dir + f"/{metric_name}_{filename}.svg")


def plot_metric_by_length(df, metric, metric_title, models, lengths, meanlineprops, medianprops, colors, plot_dir):
    plt.clf()
    ...
    for i, l in enumerate(lengths):
        for j, model in enumerate(models):
            bp1 = plt.boxplot([get_length_metric(df, f"{metric}_{model}", l)], positions=[i * 8 + j],
                              notch=False, patch_artist=True, widths=1, showmeans=True, meanprops=meanlineprops,
                              medianprops=medianprops)
            bp1['boxes'][0].set_facecolor(colors[j])
        ...
        print("Whiskers:", [item.get_ydata() for item in bp1['whiskers']])
    ticks = [str(i) for i in [1] + list(range(3, 31, 3))]
    plt.xticks([(i * 8) + len(models)/2 for i in [0] + list(range(2, 30, 3))], ticks)
    legends = []
    for i, model in enumerate(models):
        hB, = plt.plot([1, 1], '-', color=colors[i])
        legends.append(hB)
        hB.set_visible(False)
    plt.legend(legends, models, loc="lower left")
    plt.xlim(left=-5)
    plt.ylim(0, 1)
...
def plot_average_metric_by_length(df, metric, metric_title, models, lengths, colors, plot_dir):
    aurocs = {}
    for model in models:
        aurocs[model] = []
        for l in lengths:
            aurocs[model].append(np.mean(get_length_metric(df, f"{metric}_{model}", l)))
    ...
    for i, model in enumerate(aurocs.keys()):
        plt.plot(lengths, aurocs[model], linestyle="-", marker="o", color=colors[i], label=model)
...
def plot_average_metrics_by_length(df, models, lengths, colors, plot_dir):
    # Collect metrics
    aurocs = {model: [np.mean(get_length_metric(df, f"auroc_{model}", l)) for l in lengths] for model in models}
    auprcs = {model: [np.mean(get_length_metric(df, f"auprc_{model}", l)) for l in lengths] for model in models}
    mses   = {model: [np.mean(get_length_metric(df, f"mse_{model}",   l)) for l in lengths] for model in models}
    ...
    for ax, (metric_name, metric_data) in zip(axes, metric_info):
        for i, model in enumerate(models):
            ax.plot(lengths,
                    metric_data[model],
                    linestyle="-",
    ...
    #fig.suptitle("Average cv metrics grouped by sample length", fontsize=14)

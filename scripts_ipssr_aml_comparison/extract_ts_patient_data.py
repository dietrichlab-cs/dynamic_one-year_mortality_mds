...
def build_patient_data(patients, dataset_parameters, alias_parameter, data_files):
    ...
    for file in data_files:
        with open(file, encoding='iso-8859-1') as data_file:
            ...
            for row in reader:
                ...
                # get the medico/meddat ID of the current row
                meddat_id = row['Lebensnummer']
                # if the parameter or the patient is not in the respective predetermined sets, skip the row
                if (parameter_name not in dataset_parameters.keys() and parameter_name not in alias.keys()) or (meddat_id not in patients.keys()):
                    continue
                # skip entries that where taken before MDS diagnosis
                if datetime.strptime(row['Abnahmedatum'], '%d.%m.%Y') < patients[row['Lebensnummer']][0]:
                    continue
                ...
                if row['Messwert String'][0] in ['>', '<']:
                    ...
                    try: float(row['Messwert String'])
                    except: continue
                    value = row['Messwert String']
                # get the examination date
                examination_date = datetime.strptime(row['Abnahmedatum'], '%d.%m.%Y')
                ...
                if parameter_name in alias:
                    ...
                    entries[meddat_id][alias[parameter_name]].append([examination_date, value])
                else:
                    entries[meddat_id][parameter_name].append([examination_date, value])
    ...
    for patient in entries.keys():
        ...
        if len(entries[patient]) < len(dataset_parameters.keys()): continue

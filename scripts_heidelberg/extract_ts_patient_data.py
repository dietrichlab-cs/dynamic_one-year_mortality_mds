...
def build_patient_data(patients, dataset_parameters, alias_parameter, data_files):
    ...
    for file in data_files:
        with open(file, encoding="UTF-8-sig") as data_file:
            reader = csv.DictReader(data_file, delimiter=';')
            for row in reader:
                # combine Analyt name and number into a unique key
                parameter_name = row['N2LEISTID']
                ...
                if datetime.strptime(row['N2DATE_D'], '%d.%m.%Y') < patients[row['PATNR']][0]:
                    continue
                ...
                examination_date = datetime.strptime(row['N2DATE_D'], '%d.%m.%Y')

...
from scripts_mannheim.plotting.plotting_utils import precision_recall_plot, aur_roc_plot, plot_loss, \
...
def classify(pretrained_model, X_test, y_test, feature_matrix, verbose=False):
    ...
    X_test = X_test.drop(columns=['easix', 'hb_ed', 'leuko_ed'], errors='ignore')
...
def train_and_classify(pretrained_model, pretrained_model_baseline, feature_matrix_test_csv, metric_output_file):
    ...
    for l in lengths:
        ...
        if sum(y_test[sample_idx_length]) in [0, len(y_test[sample_idx_length])]:
            continue
        n_samples = len(y_test[sample_idx_length])
        n_pos_samples = sum(y_test[sample_idx_length])
        ...
        metric_output.write(f"Samples: {n_samples}, positive samples: {n_pos_samples}\n")

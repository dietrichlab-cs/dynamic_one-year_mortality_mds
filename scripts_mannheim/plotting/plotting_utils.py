...
def aur_roc_plot(estimator, X_test, y_test, plot_dir, filename=None):
    RocCurveDisplay.from_estimator(estimator, X_test, y_test, pos_label=1)
    if not filename: plt.savefig(plot_dir + "/auroc_curve.svg")
    else: plt.savefig(plot_dir + f"/{filename}.svg")
    plt.clf()

def calibration_plot(y_test, y_prob, plot_dir, filename=None):
    CalibrationDisplay.from_predictions(y_test, y_prob[:, 1])
    if not filename: plt.savefig(plot_dir + "/calibration.svg")
    else: plt.savefig(plot_dir + f"/{filename}.svg")
...
def classification_plot(test_data_df, y_test, output_file, plot_dir, max_quarters):
    ...
    # Show the plot
    plt.tight_layout()
    plt.savefig(plot_dir + "/classification" + output_file + "_plot.svg")
    plt.clf()
